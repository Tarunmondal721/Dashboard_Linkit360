...
if (isset($CountryId) && !empty($CountryId)) {
    $operators = App\Models\Operator::GetOperatorByCountryId($CountryId)->Status(1)->orderBy('operator_name', 'ASC')->get();}
...
<div class="card shadow-sm mt-0">
    <div class="card-body">
        <div class="row">
            <div class="col-md-2">
                <div class="form-group">
                    ...
                    <select name="country_id" class="form-control select2" id="dashboard-country" onchange="country()"
                        <?php echo isset($CountryId) ? 'value="' . $CountryId . '"' : ''; ?>>
            ...
            <div class="col-md-2">
                ...
                <select name="operator_id" class="form-control select2" onchange="operator()" id="dashboard-operator">
                    ...
                    @foreach ($operators as $operator)
                        <option value="{{ $operator->id_operator }}"<?php echo isset($filterOperator) && $operator->id_operator == $filterOperator ? 'selected' : ''; ?>>
            ...
            <div class="col-md-2 required has-success">
                <label for="dashboardservice">Service Name</label>
                <select name="service_id" class="form-control select2" id="dashboard-service" style="width: 100%"
                    data-select2-id="select2-data-dashboard-service" tabindex="-1" aria-hidden="true" required>
                    <option value="">Service Name</option>


                    @foreach ($services as $service)
                        <option value="{{ $service->id_service }}"
                            data-select2-id="select2-data-{{ $service->id_service }}-7afa" <?php echo isset($ServiceId) && $service->id_service == $ServiceId ? 'selected' : ''; ?>>
            ...
            <div class="col-lg-2">
                ...
                <select name="date_range" class="form-control select2" id="date_range" onchange="date_ranges()"
            ...
            <div class="col-md-4">
                ...
                <button name="detailsearch" id="detailsearch" class="btn btn-primary" onclick="submit()"><i
...
<script>
    ...
    function country() {
        ...
        $.ajax({
            ...
            success: function(responses) {
                var dashboardOperatorSelect = $('#dashboard-operator');

                dashboardOperatorSelect.empty();

                dashboardOperatorSelect.append('<option value="">Operator Name</option>');


                $.each(responses, function(index, response) {
                    dashboardOperatorSelect.append('<option value="' + response.id_operator + '">' +
                        response.operator_name + '</option>');
                });


                document.getElementById('dashboard-service').innerHTML =
                    '<option value="">Service Name</option>';
            },
        });
    }
    ...
    //                 '<option value="">Service Name</option>';
    //             $.each(responses, function(index, response) {
    //                 $("#dashboard-service").append('<option value="' + response.id_service + '">' +
    //                     response.service_name + '</option>');
    //             });
    //         },
    //     });
    // }
    ...
    function operator() {
        ...
        $.ajax({
            ...
            dataType: "json",
            success: function(responses) {
                var select = document.getElementById('dashboard-service');
                select.innerHTML = '';

                var defaultOption = document.createElement(
                    "option");
                defaultOption.value = "";
                defaultOption.text = "Service Name";
                ...
                $.each(responses, function(index, response) {
                    $("#dashboard-service").append('<option value="' + response.id_service + '">' +
                        response.service_name + '</option>');
                    if (firstService) {
                        select.value = response.id_service;
                        firstService = false;
                    }
                });
            },
