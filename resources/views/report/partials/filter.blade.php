    ...
    <div class="row">
      ...
      <div class="col-lg-2">
        ...
        <select name="report_type" class="form-control select2" required id="report_type">
          ...
          <option value="business" <?php echo isset($BusinessTypeWise) ? 'selected': '' ?>>Business Type Summary</option>
          @if($allowAllOperator)
          <option value="manager" <?php echo isset($AccountManagerWise) ? 'selected': '' ?>>Account Manager's Summary</option>
          @endif
        </select>
      </div>
      ...
      <!-- <div class="col-lg-2">
        <label class="invisible d-block">Export Excel</label>
        <form method="GET" action="{{ route('export.excel') }}" id="excelDownload">
          <button class="btn btn-sm" style="color:white; background-color:green"><i class="fa fa-file-excel-o"></i>Export All as XLS</button>
        </form>
      </div> -->
    </div>
  </div>
