        ...
        <div class="card shadow-sm ">
          ...
          <div class="d-flex align-items-center my-2">
            <span class="badge badge-secondary px-2 bg-primary">
              All Countries({{ count($sumemry) }}) | Operators({{isset($allDataSum)?$allDataSum['operator_count']:''}}) </span>
            ...
            <div class="card-text">
              <table width="100%" id="dtbl">
                ...
                <tbody>
                  ...
                  <tr>
                    <th>GMV/AVG GMV<sup><i class="ml-2 mr-2 mt-1 text-dark fa fa-info-circle" title="End User Revenue Before Share / Average End User Revenue Before Share"></i></sup></th>
                    <td class="p-1 gray-bg "><div class="inter"><span class="{{ $allDataSum['current_revenue_usd_class'] }} intermate current_revenue">{{ numberConverter($allDataSum['current_revenue_usd'],2) }}(USD) <i
                  ...
                  <tr>
                    <th>G.Rev/AVG G.Rev<sup><i class="ml-2 mr-2 mt-1 text-dark fa fa-info-circle" title="Gross Revenue / Average Gross Revenue"></i></sup></th>
                    <td class="p-1 gray-bg "><div class="inter"><span class="{{ $allDataSum['current_gross_revenue_usd_class'] }} intermate current_gross_revenue">{{ numberConverter($allDataSum['current_gross_revenue_usd'],2) }}(USD) <i
                  ...
                  <tr>
                    ...
                    <td class="p-1 gray-bg "><div class="inter"><span class="{{ $allDataSum['current_net_revenue_usd_class'] }} intermate current_net_revenue">{{ numberConverter($allDataSum['current_net_revenue_usd'],2) }}(USD) <i
                          class="fa {{ $allDataSum['current_net_revenue_usd_arrow'] }}"></i>&nbsp;</span><span class="{{ $allDataSum['estimated_avg_net_revenue_usd_class'] }}">{{ numberConverter($allDataSum['current_avg_net_revenue_usd'],2) }}(USD)<i
                          class="fa {{ $allDataSum['estimated_avg_net_revenue_usd_arrow'] }}"></i>&nbsp;</span></div></td>
                    <td class="p-1"><div class="inter"><span class="{{ $allDataSum['estimated_net_revenue_usd_class'] }} intermate">{{ numberConverter($allDataSum['estimated_net_revenue_usd'],2) }}(USD) <i
                          class="fa {{ $allDataSum['estimated_net_revenue_usd_arrow'] }}"></i>&nbsp;<small>{{ $allDataSum['estimated_net_revenue_usd_percentage'] }}%</small></span><span class="{{ $allDataSum['estimated_avg_net_revenue_usd_class'] }}">{{ numberConverter($allDataSum['estimated_avg_net_revenue_usd'],2) }}(USD)<i
                          class="fa {{ $allDataSum['estimated_avg_net_revenue_usd_arrow'] }}"></i>&nbsp;<small>{{ $allDataSum['estimated_avg_net_revenue_usd_percentage'] }}%</small></span></div></td>
                    <td class="p-1 gray-bg"><div class="inter"><span class="{{ $allDataSum['last_net_revenue_usd_class'] }} intermate">{{ numberConverter($allDataSum['last_net_revenue_usd'],2) }}(USD) <i
                          class="fa {{ $allDataSum['last_net_revenue_usd_arrow'] }}"></i>&nbsp;<small>{{ $allDataSum['last_net_revenue_usd_percentage'] }}%</small></span><span class="{{ $allDataSum['last_avg_net_revenue_usd_class'] }}">{{ numberConverter($allDataSum['last_avg_net_revenue_usd'],2) }}(USD)<i
                          class="fa {{ $allDataSum['last_avg_net_revenue_usd_arrow'] }}"></i>&nbsp;<small>{{ $allDataSum['last_avg_net_revenue_usd_percentage'] }}%</small></span></div></td>
                    <td class="p-1"><div class="inter"><span class="intermate">{{ numberConverter($allDataSum['prev_net_revenue_usd'],2) }}(USD)</span><span>{{ numberConverter($allDataSum['prev_avg_net_revenue_usd'],2) }}(USD)</span> </div></td>
                  </tr>

                  <tr>
                    <th>REG/AVG REG/C.MO<sup><i class="ml-2 mr-2 mt-1 text-dark fa fa-info-circle" title="reg / average reg / campaign MO"></i></sup></th>
                    <td class="p-1 gray-bg "><div class="inter"><span class="{{ $allDataSum['current_total_mo_class'] }} intermate current_total_mo">{{ number_format(round($allDataSum['current_total_mo'])) }}<i
                  ...
                  <tr>
                    ...
                    <td class="p-1 gray-bg current_month_costCampaign"><div class="inter"><span class="{{ $allDataSum['current_cost_class'] }} intermate current_cost">{{ numberConverter($allDataSum['current_cost'],3) }}<i
                  ...
                  <tr>
                    ...
                    <td class="p-1 gray-bg current_month_roi"><div class="inter"><span class="{{ $allDataSum['currentMonthROI_class'] }} intermate currentMonthROI">{{ numberConverter($allDataSum['currentMonthROI'],4,'pre') }}<i
                  ...
                  <tr>
                    ...
                    <td class="p-1 gray-bg "><div class="inter"><span class="{{ $allDataSum['current_pnl_class'] }} intermate current_pnl">{{ number_format(round($allDataSum['current_pnl'])) }}<i
          ...
          <div class="d-flex align-items-center my-2">
            <span class="badge badge-secondary px-2 bg-primary">
              <img src="{{ asset('/flags/'.$report['country']['flag']) }}" width="30"
                height="20">&nbsp;<a href="{{ route('report.daily.country.pnldetails','company=allcompany&country='.$report['country']->id) }}" class="text-white">
              {{$report['country']['country']}} | Operators({{isset($report['operator_count'])?$report['operator_count']:''}})

                | Last Update: {{$report['updated_at']}}</a> </span>
                <a href="#" style="color: red" class="pl-1{{$report['countNotification'] == 0 ? '' :' animated infinite jello' }}  " data-ajax-popup="true" data-title="{{__('Notification ' . $report['country']['country'] )}}" data-url="{{ URL::to('/notification/notification'.'/'.$report['country']['id']) }}">
                  <i class="fa fa-bell" style="font-size: 35px;"><sup style="font-size: 20px;">{{$report['countNotification'] == 0 ? '' : $report['countNotification']}}</sup></i> </a>
            <span class="flex-fill ml-2 bg-primary w-100 font-weight-bold" style="height: 1px;"></span>
            ...
            <div class="card-text">
              <table width="100%">
                ...
                <tbody>
                  ...
                  <tr>
                    <th>N.Rev/AVG N.Rev<sup><i class="ml-2 mr-2 mt-1 text-dark fa fa-info-circle" title="Net Revenue / Average Net Revenue"></i></sup></th>
                    <td class="p-1 gray-bg "><div class="inter"><span class="{{ $report['current_net_revenue_usd_class'] }} intermate">{{ numberConverter($report['current_net_revenue_usd'],2) }}(USD) <i
                          class="fa {{ $report['current_net_revenue_usd_arrow'] }}"></i>&nbsp;</span><span class="{{ $report['estimated_avg_net_revenue_usd_class'] }}">{{ numberConverter($report['current_avg_net_revenue_usd'],2) }}(USD)<i
                          class="fa {{ $report['estimated_avg_net_revenue_usd_arrow'] }}"></i>&nbsp;</span></div></td>
                    <td class="p-1"><div class="inter"><span class="{{ $report['estimated_net_revenue_usd_class'] }} intermate">{{ numberConverter($report['estimated_net_revenue_usd'],2) }}(USD) <i
                          class="fa {{ $report['estimated_net_revenue_usd_arrow'] }}"></i>&nbsp;<small>{{ $report['estimated_net_revenue_usd_percentage'] }}%</small></span><span class="{{ $report['estimated_avg_net_revenue_usd_class'] }}">{{ numberConverter($report['estimated_avg_net_revenue_usd'],2) }}(USD)<i
                          class="fa {{ $report['estimated_avg_net_revenue_usd_arrow'] }}"></i>&nbsp;<small>{{ $report['estimated_avg_net_revenue_usd_percentage'] }}%</small></span></div></td>
                    <td class="p-1 gray-bg"><div class="inter"><span class="{{ $report['last_net_revenue_usd_class'] }} intermate">{{ numberConverter($report['last_net_revenue_usd'],2) }}(USD) <i
                          class="fa {{ $report['last_net_revenue_usd_arrow'] }}"></i>&nbsp;<small>{{ $report['last_net_revenue_usd_percentage'] }}%</small></span><span class="{{ $report['last_avg_net_revenue_usd_class'] }}">{{ numberConverter($report['last_avg_net_revenue_usd'],2) }}(USD)<i
                          class="fa {{ $report['last_avg_net_revenue_usd_arrow'] }}"></i>&nbsp;<small>{{ $report['last_avg_net_revenue_usd_percentage'] }}%</small></span></div></td>
                    <td class="p-1"><div class="inter"><span class="intermate">{{ numberConverter($report['prev_net_revenue_usd'],2) }}(USD)</span><span>{{ numberConverter($report['prev_avg_net_revenue_usd'],2) }}(USD)</span> </div></td>
                  </tr>
                  <tr>
                    <th>REG/AVG REG/C.MO<sup><i class="ml-2 mr-2 mt-1 text-dark fa fa-info-circle" title="reg / average reg / campaign MO"></i></sup></th>
